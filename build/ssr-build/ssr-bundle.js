...
var articleList_ArticleList = function (_Component) {
	...
	ArticleList.prototype.componentDidMount = function componentDidMount() {
		...
		var oldArticles = window.localStorage.getItem('articles');
		if (oldArticles) {
			oldArticles = JSON.parse(oldArticles);
			this.setState({ articles: oldArticles });
		}
		// get new articles from api
		window.fetch('/getArticles').then(function (data) {
			return data.json();
		}).then(function (articles) {
			//do stuff
			_this2.setState({ newArticles: articles });
			var newArticles = JSON.stringify(articles);
	...
	ArticleList.prototype.render = function render(props, state) {
		...
		var mergeNewArticles = function mergeNewArticles(e) {
			var _state$articles;

			// user asked to see new articles. merge them to top of articles list
			console.log('mergeNewArticles!');
			(_state$articles = state.articles).unshift.apply(_state$articles, state.newArticles);
			_this3.setState({ newArticles: [] });
			_this3.setState({ articles: state.articles });
		};
		// const updateButtonClass = state.articles.length > 0 ? {style.show} : {style.hide};
		var updateButton = Object(preact_min["h"])(
			preact_material_components_Button,
			{ className: 'mdc-button mdc-button--raised', onClick: mergeNewArticles },
			'Load ',
...
// removed by extract-text-webpack-plugin
module.exports = {"hide":"hide__3srMN","show":"show__BcLgQ","articlelist":"articlelist__1sUyM"};
...
module.exports = map;
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__("ENL0")(module)))
